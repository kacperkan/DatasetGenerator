...
void DatasetGeneratorTransparent_t::opBlur(cv::Mat& m) {
    ...
    m_prngValue = (m_prngValue % 2) == 0
                      ? m_prngValue + 1
                      : m_prngValue;  // Generate even number (eg. 3, 5, 7, ...)
...
void DatasetGeneratorTransparent_t::opRotateZ(cv::Mat& m, cv::Mat& alpha) {
    ...
    if (m_prngProbability <= 20)  // 20%
    {
        ...
        ImageProcessing::rotateImage(m, m, 90, 90, 90 + m_prngValue, 0, 0, 200,
                                     200);
        ImageProcessing::rotateImage(alpha, alpha, 90, 90, 90 + m_prngValue, 0,
                                     0, 200, 200);
    }
#endif
}
...
void DatasetGeneratorTransparent_t::opRotateY(cv::Mat& m, cv::Mat& alpha) {
    ...
    if (m_prngProbability <= 20)  // 20%
    {
        ...
        ImageProcessing::rotateImage(m, m, 90, 90 + m_prngValue, 90, 0, 0, 200,
                                     200);
        ImageProcessing::rotateImage(alpha, alpha, 90, 90 + m_prngValue, 90, 0,
                                     0, 200, 200);
    }
#endif
}
...
#endif
{
    ...
    {
            m.convertTo(m, CV_32FC3);                // Convert to floating
    point, 3 channels cv::divide(m, divide, m);                // Divide
    (decrease brightness & contrast) m.convertTo(m, CV_8UC3);                 //
    Convert back to unsigned integer, 3 channels
    }*/
#endif
}
...
void DatasetGeneratorTransparent_t::opAddGradient(cv::Mat& m) {
    ...
    if (m_prngProbability <= 25)  // 25%
    {
        ...
        int dir = (m_probability(m_rng) < 50) ? m_distGradient(m_rng)
                                              : -m_distGradient(m_rng);

        cv::Mat tmp =
            cv::Mat::zeros(cv::Size(2 * m.cols, 2 * m.rows), m.type());
        int x_coord = m.cols / 2;
        int y_coord = m.rows / 2;

        m.copyTo(tmp(cv::Rect(x_coord, y_coord, m.cols, m.rows)));

        ImageProcessing::rotateImage(tmp, tmp, 90, 90, 90 + dir, 0, 0, 200,
                                     200);

        cv::Mat newImage;
        tmp.copyTo(newImage);

        // Do the operation for each pixel: newImage(i,j) = alpha*img(i,j) +
        // beta
        for (int y = 0; y < tmp.rows; y++) {
            for (int x = 0; x < tmp.cols; x++) {
                for (int c = 0; c < 3; c++) {
                    newImage.at<cv::Vec3b>(y, x)[c] = cv::saturate_cast<uchar>(
                        alpha * (tmp.at<cv::Vec3b>(y, x)[c]) + beta);
                }
            }
        ...
        ImageProcessing::rotateImage(newImage, newImage, 90, 90, 90 - dir, 0, 0,
                                     200, 200);
...
///////////////////////////////////////////////////////////////////////////////////////////////
#ifdef ROI_SELECTION
void DatasetGeneratorTransparent_t::generateDataset(const ROIBuffer_t& b,
                                                    cv::Mat m, cv::Mat m2)
#else
...
#ifdef REALISTIC_LIGHTNING
    opAdjustBrightness(m2, ImageProcessing::getImgBrightness(
                               m(cv::Rect{x, y, m2.cols, m2.rows})));
#else
