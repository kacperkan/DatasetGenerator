...
void rotateImage(const cv::Mat& input, cv::Mat& output, double alpha,
    ...
    cv::Mat A1 = (cv::Mat_<double>(4, 3) << 1, 0, -w / 2, 0, 1, -h / 2, 0, 0, 0,
                  0, 0, 1);

    // Rotation matrices around the X, Y, and Z axis
    cv::Mat RX = (cv::Mat_<double>(4, 4) << 1, 0, 0, 0, 0, cos(alpha),
                  -sin(alpha), 0, 0, sin(alpha), cos(alpha), 0, 0, 0, 0, 1);

    cv::Mat RY = (cv::Mat_<double>(4, 4) << cos(beta), 0, -sin(beta), 0, 0, 1,
                  0, 0, sin(beta), 0, cos(beta), 0, 0, 0, 0, 1);

    cv::Mat RZ = (cv::Mat_<double>(4, 4) << cos(gamma), -sin(gamma), 0, 0,
    ...
    // Translation matrix
    cv::Mat T = (cv::Mat_<double>(4, 4) << 1, 0, 0, dx, 0, 1, 0, dy, 0, 0, 1,
                 dz, 0, 0, 0, 1);

    // 3D -> 2D matrix
    cv::Mat A2 =
        (cv::Mat_<double>(3, 4) << f, 0, w / 2, 0, 0, f, h / 2, 0, 0, 0, 1, 0);
...
void copy2bgCropped(cv::Mat& bg, cv::Mat& img, cv::Mat& annot, const int& x,
    ...
    if (useMask) {
        ...
        ////////////// Method 1 ////////////// (More suitable for more than 4
        /// points)
        /*cv::Point poly[1][4];
        ...
        cv::fillPoly(imgMask, polygons, numPoints, 1, cv::Scalar(255, 255,
        255));*/
        ...
        cv::rectangle(imgMask, cv::Point(diffx, diffy),
                      cv::Point(diffx + sizex, diffy + sizey),
    ...
    cv::Mat clone = bg;
    cv::seamlessClone(img, bg, imgMask, center, clone,
                      cv::NORMAL_CLONE); /* MIXED_CLONE */
    bg = std::move(clone);
#else
...
static bool isRedHue(const uchar& hue) {
    return ((hue >= 0 && hue <= 9) || (hue >= 177 && hue <= 180));
}
