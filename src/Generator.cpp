...
///////////////////////////////////////////////////////////////////////////////////////////////
void DatasetGenerator_t::adjustPosition(const cv::Mat& m, cv::Mat& m2,
                                        cv::Mat& a, int& x, int& y) {
    ...
    if (((x + m2.cols) >= m.cols) || ((y + m2.rows) >= m.rows)) {
        ...
        if ((overlap_X < (m2.cols / 2) || overlap_Y < (m2.rows / 2)) &&
            m_probability(m_rng) >= 10)  // 90%
        {
#endif
            // Shift image left
            if ((x + m2.cols) > m.cols) {
...
///////////////////////////////////////////////////////////////////////////////////////////////
void DatasetGenerator_t::showBbox(cv::Mat& m, const cv::Mat& m2, const int& x,
                                  const int& y) {
#ifdef GENERATOR_DEBUG
    int x2 =
        ((x + ((double)m2.cols / 2)) / m.cols) * m.cols - ((double)m2.cols / 2);
    int y2 =
        ((y + ((double)m2.rows / 2)) / m.rows) * m.rows - ((double)m2.rows / 2);
