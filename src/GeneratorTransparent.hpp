...
/**
 * @brief Class providing generating of images and its annotation using
 * transparent TS's
 */
class DatasetGeneratorTransparent_t : public DatasetGenerator_t {
   public:
    ...
    /************************************************************************************
     * @brief Generating of images & annotation (Image of traffic sign with
     * alpha channel)
     *
     ...
     */
#ifdef ROI_SELECTION
    virtual void generateDataset(const ROIBuffer_t& b, cv::Mat m,
                                 cv::Mat m2) override;
#else
   ...
   protected:
     ...
     *
     * @param m [in, out] Modified image
     * @param factor [in] Lightning factor (just in case REALISTIC_LIGHTNING is
     * defined)
     */
    ...
    /************************************************************************************
     * @brief Add salt and pepper (blue and white pixels) noise to the traffic
     * sign
     *
     * @param m [in, out] Modified image
     */
   ...
   private:
    ...
    /** @brief Pseudo-random uniform distribution for gradient intensity and
     * direction */
    PRNG::Uniform_t m_distGradient;
    /** @brief Pseudo-random uniform distribution for brightness & contrast
     * modification */
